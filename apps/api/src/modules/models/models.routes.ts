...
export const modelsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/models/discover", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!providerId || !baseUrl) {
      return reply.code(400).send({ error: "providerId and baseUrl are required" });
    }
    ...
    try {
      ...
      if (err.name === "TimeoutError" || err.code === "UND_ERR_CONNECT_TIMEOUT") {
        return reply.code(504).send({ error: "Provider API timed out" });
      }
      return reply.code(502).send({ error: `Failed to fetch models: ${err.message}` });
    }
  });
  ...
  fastify.get("/models/changelog", async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(500).send({ error: `Failed to fetch changelog: ${err.message}` });
    }
  });
  ...
  fastify.post("/models/changelog/apply", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (!Array.isArray(providerIds) || providerIds.length === 0) {
        return reply.code(400).send({ error: "providerIds array is required" });
      }
      ...
      return reply.code(500).send({ error: `Failed to apply changes: ${err.message}` });
    }
  });
  ...
  fastify.get("/models/snapshot/:providerId", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (result.rows.length === 0) {
        return reply.code(404).send({ error: "No snapshot found for this provider" });
      }

      return { providerId: result.rows[0].provider_id, models: result.rows[0].models, updatedAt: result.rows[0].updated_at };
    } catch (err: any) {
      return reply.code(500).send({ error: `Failed to fetch snapshot: ${err.message}` });
    }
  });
};