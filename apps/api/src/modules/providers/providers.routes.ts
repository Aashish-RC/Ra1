...
export const providersRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get("/providers", async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(500).send({ error: `Failed to fetch provider configs: ${err.message}` });
    }
  });
  ...
  fastify.get("/providers/:providerId", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (result.rows.length === 0) {
        return reply.code(404).send({ error: "Provider config not found" });
      }
      return result.rows[0];
    } catch (err: any) {
      return reply.code(500).send({ error: `Failed to fetch provider config: ${err.message}` });
    }
  });
  ...
  fastify.patch("/providers/:providerId", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(500).send({ error: `Failed to update provider config: ${err.message}` });
    }
  });
};